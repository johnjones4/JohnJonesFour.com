...
module.exports = function(eleventyConfig) {
    ...
    // Custom permalink for blog posts
    collection.items = collection.items.map(function (post) {
      if (post.url.startsWith('/blog') && post.url !== '/blog/') {
				const year = post.date.getFullYear();
				const month = (post.date.getMonth() + 1).toString().padStart(2, '0');
				const day = post.date.getDate().toString().padStart(2, '0');
				const path = `/${year}/${month}/${day}/${post.fileSlug}/`;
				post.data.permalink = path;
				post.url = path;
				post.page.url = path
				post.filePathStem = path;
				post.outputPath = `_site${path}index.html`
				post.page.outputPath = `_site${path}index.html`
			}
			return post
    });
    return collection;
